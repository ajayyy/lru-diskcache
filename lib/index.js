...
DiskCache.prototype.reset = function() {
    ...
    if (fs.existsSync(this.path)) {
        ...
        for (const file of files) {
            const size = fs.statSync(path.join(this.path, file)).size
            this.lru.set(this._keyFromFilename(file), size);
        }
    } else {
...
DiskCache.prototype.get = function(key, opts) {
    ...
    this.lru.get(key);
    return this._read(key, { encoding: opts.encoding });
};
...
DiskCache.prototype._keyFromFilename = function(filename) {
